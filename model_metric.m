classdef model_metric < handle
    ...
    methods
        ...
        function correlation_analysis(obj, varargin)
            % Can be called as obj.correlation_analysis('GitHub')
            % obj.correlation_analysis('GitHub','MATC')
            % obj.correlation_analysis('GitHub','Tutorial','sourceforge','matc','Others')
            ...
            where_clause = ' ) where is_lib = 0 and compiles = 1 and CComplexity !=-1 Order by CComplexity';
           ...
           if nargin > 1
                for i = 2:nargin-1
                    innerquery = [innerquery,' union select b.*, a.cnt from  ',varargin{i}, '_Metric b join ( select File_id, Model_Name, count(blk_type) as cnt from ',varargin{i}, ...
                    '_Block_Info group by File_id,Model_Name) a on a.File_id = b.File_id and  a.Model_Name = b.Model_Name'];
        ...
        function analyze_metrics(obj,flag)
                ...
                total_analyzed_mdl_query = ['select count(*) from',...
                                ' (select * from github_metric where  is_lib=0',...
                                 '   union',...
                                  ...
                                  '   union',...
                                  '  select * from  sourceforge_metric where  is_lib=0',...
                                  '   union',...
                                  '  select * from  tutorial_metric where  is_lib=0',...
                                    '   union',...
                                  '  select * from  others_metric where  is_lib=0',...
                                   ' )'];
            else
                total_analyzed_mdl_query = ['select count(*) from',...
                                ' (select * from github_metric where  is_lib=0',...
            ...
            if flag
                ...
                'not in (select file_id,Model_Name from tutorial_Metric where is_lib=1)',...
                 ' union',...
                ' select * from sourceforge_Subsys_Info where (file_id,Model_Name)', ...
                'not in (select file_id,Model_Name from sourceforge_Metric where is_lib=1)',...
                 ' union',...
                ' select * from others_Subsys_Info where (file_id,Model_Name)', ...
                'not in (select file_id,Model_Name from others_Metric where is_lib=1)',...
                ')'];
            else
                query_hierar_median_blk_cnt =['select depth,block_count from ',...
                '(select * from GitHub_Subsys_Info where (file_id,Model_Name)',...
            ...
            ' union',...
            ' select count(*) as c from  sourceforge_metric where SCHK_block_count>1000 and is_lib=0',...
            ' union',...
            ' select count(*) as c from  tutorial_metric where SCHK_block_count>1000 and is_lib=0',...
                      ' union',...
            ' select count(*) as c from  others_metric where SCHK_block_count>1000 and is_lib=0',...
            ' )'
            ];
            else
            models_over_1000_blk_query = ['select sum(c) from(',...
                ...
                models_use_mdlref_query = ['select mdlref_nam_count from',...
                                ' (select * from github_metric where unique_mdl_ref_count>0 and  is_lib=0',...
                                 '   union',...
                                  ...
                                  '   union',...
                                  '  select * from  sourceforge_metric where unique_mdl_ref_count>0 and is_lib=0',...
                                  '   union',...
                                  '  select * from  tutorial_metric where unique_mdl_ref_count>0 and is_lib=0',...
                                      '   union',...
                                  '  select * from  others_metric where unique_mdl_ref_count>0 and is_lib=0',...
                                   ' )'];
            else
            models_use_mdlref_query = ['select mdlref_nam_count from',...
                                ' (select * from github_metric where unique_mdl_ref_count>0 and  is_lib=0',...
                                ...
                                ' (select * from github_metric where unique_sfun_count>0 and  is_lib=0',...
                                 '   union',...
                                  '  select * from  matc_metric where unique_sfun_count>0 and is_lib=0',...
                                   '   union',...
                                  '  select * from  tutorial_metric where unique_sfun_count>0 and is_lib=0',...
                                   '   union',...
                                  '  select * from  sourceforge_metric where unique_sfun_count>0 and is_lib=0',...
                                      '   union',...
                                  '  select * from  others_metric where unique_sfun_count>0 and is_lib=0',...
                                   ' )'];
            else
            ...
            switch choice
                case 'Simple'
                    simple_union_table_query = [
                            ' (select * from Github_metric where File_id in (select id from GitHub_Projects where forks_count<10 or stargazers_count<10) and  is_Lib = 0',... 
                            ' union', ...
                            ' select * from Matc_metric where File_id in (select id from matc_Projects where no_of_ratings<10 or downloads<10 or average_rating<3.0) and  is_Lib = 0 )']
                ...
                case 'Advanced'
                    advanced_union_table_query = [
                            ' (select * from Github_metric where File_id in (select id from GitHub_Projects where forks_count>=10 and stargazers_count>=10) and  is_Lib = 0',... 
                            ' union', ...
                            ' select * from Matc_metric where File_id in (select id from matc_Projects where no_of_ratings>=10 and downloads>=10 and average_rating>=3.0) and  is_Lib = 0 )']
                    ...
                    switch lower(choice)
                        ...
                        case 'sourceforge_metric'
                             most_frequentlused_blocks_query = ['select BLK_TYPE,sum(count)  as c from sourceforge_Block_Info where (File_id,Model_Name) in ( ',...
                                 ' select File_id,Model_Name from sourceforge_Metric where is_Lib=0 )',...
                                 '  group by BLK_TYPE order by  c desc'];
                       case 'tutorial_metric'
                             most_frequentlused_blocks_query = ['select BLK_TYPE,sum(count)  as c from tutorial_Block_Info where (File_id,Model_Name) in ( ',...
                                 ' select File_id,Model_Name from tutorial_Metric where is_Lib=0 )',...
                                 '  group by BLK_TYPE order by  c desc'];
                       case 'others_metric'
                             most_frequentlused_blocks_query = ['select BLK_TYPE,sum(count)  as c from others_Block_Info where (File_id,Model_Name) in ( ',...
                                 ' select File_id,Model_Name from others_Metric where is_Lib=0 )',...
                                 '  group by BLK_TYPE order by  c desc'];
                    end 
        ...
        function res = grand_total_analyze_metric(obj,flag)
            ...
            if(flag)
                ...
                others  = obj.total_analyze_metric('others_metric');
