classdef model_metric < handle
    ...
    methods
        ...
        function analyze_metrics(obj,flag)
                ...
                total_analyzed_mdl_query = ['select count(*) from',...
                                ' (select * from github_metric where  is_lib=0 and is_test = -1 ',...
                                 '   union',...
                                  ...
                                  '   union',...
                                  '  select * from  sourceforge_metric where  is_lib=0 and is_test = -1 ',...
                                  '   union',...
                                  '  select * from  tutorial_metric where  is_lib=0 and is_test = -1 ',...
                                    '   union',...
                                  '  select * from  others_metric where  is_lib=0 and is_test = -1 ',...
                                   ' )'];
            else
                total_analyzed_mdl_query = ['select count(*) from',...
                                ' (select * from github_metric where  is_lib=0 and is_test = -1 ',...
            ...
            ' union',...
            ' select count(*) as c from  sourceforge_metric where SCHK_block_count>1000 and is_lib=0 and is_test = -1 ',...
            ' union',...
            ' select count(*) as c from  tutorial_metric where SCHK_block_count>1000 and is_lib=0 and is_test = -1 ',...
                      ' union',...
            ' select count(*) as c from  others_metric where SCHK_block_count>1000 and is_lib=0 and is_test = -1 ' ,...
            ' )'
            ];
            else
            models_over_1000_blk_query = ['select sum(c) from(',...
                ...
                models_use_mdlref_query = ['select mdlref_nam_count from',...
                                ' (select * from github_metric where unique_mdl_ref_count>0 and  is_lib=0 and is_test = -1 ',...
                                 '   union',...
                                  ...
                                  '   union',...
                                  '  select * from  sourceforge_metric where unique_mdl_ref_count>0 and is_lib=0 and is_test = -1 ',...
                                  '   union',...
                                  '  select * from  tutorial_metric where unique_mdl_ref_count>0 and is_lib=0 and is_test = -1 ',...
                                      '   union',...
                                  '  select * from  others_metric where unique_mdl_ref_count>0 and is_lib=0 and is_test = -1 ',...
                                   ' )'];
            else
            models_use_mdlref_query = ['select mdlref_nam_count from',...
                                ' (select * from github_metric where unique_mdl_ref_count>0 and  is_lib=0 and is_test = -1 ',...
                                ...
                                ' (select * from github_metric where unique_sfun_count>0 and  is_lib=0 and is_test = -1 ',...
                                 '   union',...
                                  '  select * from  matc_metric where unique_sfun_count>0 and is_lib=0 and is_test = -1 ',...
                                   '   union',...
                                  '  select * from  tutorial_metric where unique_sfun_count>0 and is_lib=0 and is_test = -1 ',...
                                   '   union',...
                                  '  select * from  sourceforge_metric where unique_sfun_count>0 and is_lib=0 and is_test = -1 ',...
                                      '   union',...
                                  '  select * from  others_metric where unique_sfun_count>0 and is_lib=0 and is_test = -1 ',...
                                   ' )'];
            else
        ...
        function res = total_analyze_metric(obj,choice)
                    ...
                    switch lower(choice)
                       ...
                       case 'others_metric'
                             most_frequentlused_blocks_query = ['select BLK_TYPE,sum(count)  as c from others_Block_Info where (File_id,Model_Name) in ( ',...
                                 ' select File_id,Model_Name from others_Metric where is_Lib=0 and is_test = -1  )',...
                                 '  group by BLK_TYPE order by  c desc'];
           ...
           fprintf("%d & %d & %d & %d & %d & %d & %d & %d & %d & %d & %d & %d & %d & %d & %d",...
               res.analyzedmdl,res.mdl_compiles,res.total_hierar,res.slchk,res.connec,res.hconnec,res.fix,res.var,res.other_solver,...
               res.normal,res.ext,res.pil,res.acc,res.rpdacc,res.other_sim)
